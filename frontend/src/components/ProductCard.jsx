    ...
    return product && ( 
        <div onClick={()=> {navigate(`/products/${(Array.isArray(product.category) ? product.category[0] : product.category).toLowerCase()}/${product._id}`); scrollTo(0,0)}} className={`w-full border border-[#e5d8cc] rounded-xl bg-[var(--app-cream)] px-3 py-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${compact ? 'flex h-[315px] min-w-0 max-w-none flex-col' : 'md:px-4 min-w-56 max-w-56'}`}>
            ...
            <div className={`text-gray-500/60 ${compact ? 'flex min-h-0 flex-1 flex-col pb-2 text-xs' : 'text-sm'}`}>
                <p className={`mt-2 truncate ${compact ? 'hidden' : ''}`}>{Array.isArray(product.category) ? product.category.join(', ') : product.category}</p>
                <p className={`block w-full truncate font-medium text-gray-700 ${compact ? 'mt-2 min-h-5 text-sm' : 'text-lg'}`}>{product.name}</p>
                ...
                <div className={`flex items-end justify-between gap-2 ${compact ? 'mt-auto pb-1 pt-3' : 'mt-3'}`}>
                    ...
                    <div onClick={(e) => {e.stopPropagation(); }} className="text-primary">
                        {!cartItems[product._id] ? (
                            ...
                            <div className={`flex items-center justify-between overflow-hidden rounded bg-primary/25 select-none ${compact ? 'h-8 w-[58px] text-xs' : 'md:w-20 w-16 h-[34px]'}`}>
                                <button onClick={() => {removeFromCart(product._id)}} className="h-full cursor-pointer px-1 text-md" >
                                    -
                                </button>
                                <span className="w-5 text-center">{cartItems[product._id]}</span>
                                <button onClick={() => {addToCart(product._id)}} className="h-full cursor-pointer px-1 text-md" >
                                    +
                                </button>
                            </div>
                        )}
